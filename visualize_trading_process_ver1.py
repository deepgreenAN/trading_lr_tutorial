import sys
sys.path.append(r"E:\trading_rl_tutorial\pfrl")
...
def plot_trading_process_matplotlib(state_list, info_list, env, save_path=None, is_save=True):
    # データの取り出し
    ...
    ax2.set_ylim(0, max(stock_number_array)*5)
    ...
    ax7.plot(x, adjusted_gein_reward_array, label="adjusted gein reward", color="C3")
    ax7.plot(x, adjusted_price_reward_array, label="adjusted price reward", color="C2")
    ax7.plot(x, adjusted_penalty_array, label="adjusted penalty", color="C4")
    ax7.plot(x, reward_array, label="reward", color="C5")
    ...
    ax7.set_ylabel("reward [adjusted yen]")
    ax7.set_title("reward")

    # 全財産・現金の推移
    ax8 = fig.add_subplot(515)
    ax8.plot(x, all_property_array, color="C6", label="all property")
    ax8.axhline(y=all_property_array[0], color="black", linestyle="dashed", label="initial all property")
    ax8.set_ylabel("all property [yen]")

    ax9 = ax8.twinx()
    ax9.plot(x, cash_array, color="C7", label="cash")
    ax9.axhline(y=cash_array[0], color="gray", linestyle="dashed", label="initial cash")
    ax9.set_ylabel("cash [yen]")
    ax9.set_title("all property")
...
def plot_trading_process_bokeh(state_list, info_list, env, save_path=None, is_save=True, is_html=False):
    # データの取り出し
    cash_array = np.array(list(map(lambda state: state.cash, state_list)))
    price_array = np.array(list(map(lambda state: state.now_price, state_list)))
    stock_number_array = np.array(list(map(lambda state: state.unit_number*env.one_unit_stocks, state_list)))
    ...
    p1.extra_y_ranges = {"stock_number": Range1d(start=0, end=4*np.max(stock_number_array))}
    ...
    # figure 4
    p4 = bokeh.plotting.figure(plot_width=1200,plot_height=300,title="全体報酬・調整済み各報酬")
    p4.line(x=x,y=adjusted_gein_reward_array,line_color=bokeh_colors["C3"],line_width=2,legend_label="adjusted gein reward")
    p4.line(x=x,y=adjusted_price_reward_array,line_color=bokeh_colors["C2"],line_width=2,legend_label="adjusted price reward")
    p4.line(x=x,y=adjusted_penalty_array,line_color=bokeh_colors["C4"],line_width=2,legend_label="adjusted penalty")
    p4.line(x=x,y=reward_array,line_color=bokeh_colors["C5"],line_width=2,legend_label="reward")
    p4.line(x=[0,len(x)],y=[0,0],line_color="black",line_width=2,line_dash="dashed")
    ...
    p4.yaxis[0].axis_label = "報酬(調整) [円]"

    # figure 5
    p5 = bokeh.plotting.figure(plot_width=1200,plot_height=300,title="全財産")
    p5.line(x=x,y=all_property_array,line_color=bokeh_colors["C6"],line_width=2,legend_label="all property")
    p5.line(x=[0,len(x)],y=[all_property_array[0],all_property_array[0]],line_color="black",line_width=2,line_dash="dashed",legend_label="initial all property")
    y_min, y_max = make_y_limit(all_property_array)
    p5.y_range = Range1d(start=y_min, end=y_max)

    y_min, y_max = make_y_limit(cash_array)
    p5.extra_y_ranges = {"cash": Range1d(start=y_min,end=y_max)}
    p5.add_layout(LinearAxis(y_range_name="cash"), 'right')
    p5.line(x=x,y=cash_array,line_color=bokeh_colors["C7"],line_width=2,legend_label="cash",y_range_name="cash")
    p5.line(x=[0,len(x)],y=[cash_array[0],cash_array[0]],line_color="gray",line_width=2,line_dash="dashed",y_range_name="cash",legend_label="initial cash")

    p5.yaxis[0].axis_label = "全財産 [円]"
    p5.yaxis[1].axis_label = "現金 [円]"

    created_figure = bokeh.layouts.column(p1,p2,p3,p4,p5)
